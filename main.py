...
async def format_notes_with_gemini(raw_notes: str) -> str:
    ...
    if not GEMINI_API_KEY or not raw_notes:
        return raw_notes
    prompt = (
        "あなたは相続手続き事務所のスタッフです。面談議事録を読み取り、指定のフォーマットに整理してください。\n\n"
        "【厳守ルール】\n"
        "1. 出力はフォーマットのテキストをそのまま使い、各項目の「：」の後に議事録から読み取った内容を記入する\n"
        "2. MarkdownのヘッダーやBold（##、**）は一切使わない\n"
        "3. フォーマットの見出し（◇初動指示、１．相続関係、工程①など）はそのまま残す\n"
        "4. 議事録に記載がない項目は「要確認」と記入する\n"
        "5. フォーマット外の情報は「◇その他」欄にまとめる\n"
        "6. 余計な説明文・前置き・後書きは不要。フォーマットのテキストのみを出力する\n\n"
