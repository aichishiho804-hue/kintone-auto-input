...
async def format_notes_with_gemini(raw_notes: str) -> str:
    ...
    prompt = (
        ...
        "面談議事録を法律家の視点で読み解き、指定のフォーマットに正確・適格に整理してください。\n\n"
        "【整理の方針】\n"
        ...
        "【厳守ルール】\n"
        "1. 出力はフォーマットのテキストをそのまま使い、各項目の「：」の後に内容を記入する\n"
        "2. MarkdownのヘッダーやBold（##、**、###）は一切使わない\n"
        "3. フォーマットの見出し（◇初動指示、１．相続関係、工程①など）はそのまま残す\n"
        "4. 議事録に記載がない項目は「要確認」と記入する\n"
        "5. フォーマット外の重要情報は「◇その他」欄にまとめる\n"
        "6. 余計な説明文・前置き・後書きは一切不要。フォーマットのテキストのみを出力する\n\n"
        f"【フォーマット（このまま埋めてください）】\n{KOUTE_TEMPLATE}\n\n"
        f"【面談議事録】\n{raw_notes[:8000]}"
    )
