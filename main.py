...
async def find_box_folder_by_name(parent_id: str, name: str) -> Optional[str]:
    ...
    if r.status_code == 200:
        for item in r.json().get("entries", []):
            # parent IDは整数/文字列どちらで返ってくるかAPIバージョンによって異なるため str() で統一
            if item.get("name") == name and str(item.get("parent", {}).get("id", "")).replace(" ", "") == str(parent_id).replace(" ", ""):
                return item["id"]
...
async def format_notes_with_gemini(raw_notes: str) -> str:
    ...
    if not GEMINI_API_KEY or not raw_notes:
        return raw_notes
    prompt = (
        "あなたは相続・不動産登記を専門とする司法書士事務所のベテランスタッフです。\n"
        "面談議事録を法律家の視点で読み解き、下記フォーマットに正確・適格に整理してください。\n\n"
        "【整理の方針】\n"
        "・法律的に重要な事実（相続人の確定、財産の種類・評価額、登記の要否など）を優先して記載する\n"
        "・曖昧な表現は避け、人間が読んで即座に理解できる簡潔・正確な文章にまとめる\n"
        "・固有名詞（人名・金融機関名・不動産所在地など）は議事録の表記のまま正確に転記する\n"
        "・数字・日付・金額は省略せず正確に記載する\n\n"
        "【厳守ルール】\n"
        "1. 下記フォーマットの各行をそのまま出力し、「項目名：」の後に内容を記入する\n"
        "2. フォーマットの改行・空行・行構成を一切変えない。各項目は必ず別の行に出力する\n"
        "3. MarkdownのヘッダーやBold（##、**、###、---）は一切使わない\n"
        "4. 議事録に記載がない項目は「要確認」と記入する\n"
        "5. フォーマット外の重要情報は「◇その他、特記事項、案件固有事情など」欄にまとめる\n"
        "6. 前置きや後書き、説明文は一切不要。フォーマットのテキストのみを出力する\n\n"
        "【フォーマット（各行・空行をそのまま維持して埋めてください）】\n"
        f"{KOUTE_TEMPLATE}\n\n"
        "【面談議事録】\n"
        f"{raw_notes[:8000]}"
    )
